...
- Status: Local dependency-manifest risk scanner implemented with curated, deterministic offline rules and now participates by default in `scan` and `findings collect` runs when no explicit `--scanner` override is provided.
